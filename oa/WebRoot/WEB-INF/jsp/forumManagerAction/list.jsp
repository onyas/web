...
<div id="MainArea">
    <table cellspacing="0" cellpadding="0" class="TableStyle">
        ...
        <s:iterator value="#forumList" status="status">
			<tr class="TableDetail1 template">
				...
				<td>${description}&nbsp;</td>
				<td>
					<s:a action="forumManagerAction_delete?id=%{id}" onclick="return delConfirm()">删除</s:a>
					<s:a action="forumManagerAction_editUI?id=%{id}">修改</s:a>
					...
					<s:else>
						<s:a action="forumManagerAction_moveUp?id=%{id}">上移</s:a>
					</s:else>
					...
					<s:else>
						<s:a action="forumManagerAction_moveDown?id=%{id}">下移</s:a>
					</s:else>
    ...
    <div id="TableTail">
        <div id="TableTail_inside">
            <s:a action="forumManagerAction_addUI"><img src="${pageContext.request.contextPath}/style/images/createNew.png" /></s:a>
        </div>
    </div>
</div>
